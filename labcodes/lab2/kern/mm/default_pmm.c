...
default_init_memmap(struct Page *base, size_t n) {
    ...
    for (; p != base + n; p ++) {
        ...
        p->flags = p->property = 0; // 'PG_property' ?
        set_page_ref(p, 0);
    }
    base->property = n; // total number of pages in the block
    SetPageProperty(base);
    nr_free += n;
    list_add_before(&free_list, &(base->page_link));    // link to free_list
}
...
default_alloc_pages(size_t n) {
    ...
    if (page != NULL) {
        if (page->property > n) {
            ...
            SetPageProperty(p);
            list_add_after(&(page->page_link), &(p->page_link));    // add remain to free_list
        }
